#!/usr/bin/env python3
"""
Post articles to Hashnode using API
"""
import requests
import json
import time

HASHNODE_API_KEY = "806dbd70-600f-42d8-95d8-093f0e5a9602"
    ...
    for i, article in enumerate(ARTICLES, 1):
        try:
            ...
            if "data" in result and "createPublicationPost" in result["data"]:
                ...
                print(f"[{i}/{len(ARTICLES)}] Published: {post['url']}")
            elif "errors" in result:
                print(f"[{i}/{len(ARTICLES)}] Error: {result['errors'][0]['message']}")
            else:
                print(f"[{i}/{len(ARTICLES)}] Unknown response: {json.dumps(result)[:100]}")
        except Exception as e:
            print(f"[{i}/{len(ARTICLES)}] Exception: {e}")
